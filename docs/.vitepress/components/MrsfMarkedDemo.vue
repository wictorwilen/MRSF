...
async function renderDemo() {
  ...
  const [{ Marked }, markedMrsfMod] = await Promise.all([
    import("marked"),
    import("../../../plugins/marked/dist/browser.js"),
  ]);

  if (!MrsfControllerClass) {
    const mod = await import("../../../plugins/marked/dist/controller.js");
    MrsfControllerClass = (mod as any).MrsfController;
  }

  const parser = new Marked();
  parser.use(markedMrsfMod.markedMrsf({
    comments: sampleSidecar,
    showResolved: showResolved.value,
    lineHighlight: lineHighlight.value,
  }));

  rendered.value = String(await parser.parse(sampleMarkdown));
...
<template>
  <div class="mrsf-demo">
    <div class="mrsf-demo-controls">
      ...
      <label>
        <input v-model="lineHighlight" type="checkbox" />
        Line highlight
      </label>
    </div>
...
<style>
@import "../../../plugins/marked/dist/style.css";

...
interface MilkdownMrsfDialogThemeOptions {
  targetDocument?: Document;
  themeSource?: HTMLElement | null;
}

interface MilkdownMrsfDialogFormResult {
  text: string;
  type: Comment["type"] | null;
  severity: Comment["severity"] | null;
}

interface MilkdownMrsfFormDialogOptions extends MilkdownMrsfDialogThemeOptions {
...
interface MilkdownMrsfConfirmDialogOptions extends MilkdownMrsfDialogThemeOptions {
  title: string;
  message: string;
  confirmLabel: string;
}
...
function injectStyles(targetDocument: Document): void {
  if (stylesInjected || targetDocument.getElementById("mrsf-milkdown-dialog-styles")) {
    stylesInjected = true;
    return;
  }

  const style = targetDocument.createElement("style");
  style.id = "mrsf-milkdown-dialog-styles";
  style.textContent = `
...
function resolveDocument(options: MilkdownMrsfDialogThemeOptions): Document | null {
...
function applyThemeVariables(overlay: HTMLElement, themeSource?: HTMLElement | null): void {
  if (!themeSource || typeof getComputedStyle === "undefined") {
    return;
  }
...
export async function openMilkdownMrsfFormDialog(options: MilkdownMrsfFormDialogOptions): Promise<MilkdownMrsfDialogFormResult | null> {
  ...
  return new Promise((resolve) => {
    ...
    let settled = false;
    const close = (accepted: boolean, result: MilkdownMrsfDialogFormResult | null = null): void => {
      if (settled) {
        return;
      }
    ...
    form.addEventListener("submit", (event) => {
      ...
      close(true, {
        text,
        type: (typeSelect.value || null) as MilkdownMrsfDialogFormResult["type"],
        severity: (severitySelect.value || null) as MilkdownMrsfDialogFormResult["severity"],
      });
    });
...
export async function openMilkdownMrsfConfirmDialog(options: MilkdownMrsfConfirmDialogOptions): Promise<boolean> {
  const targetDocument = resolveDocument(options);
  if (!targetDocument) {
    return false;
  }

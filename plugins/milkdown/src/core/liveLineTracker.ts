...
function transformPoint(point: EditorPoint, change: EditorContentChange, affinity: "start" | "end"): EditorPoint {
  ...
  return affinity === "start" ? { ...start } : { ...replacementEnd };
}
...
function updateInlineComment(comment: Comment, change: EditorContentChange): boolean {
  ...
  const startPoint = { lineIndex: comment.line - 1, column: comment.start_column };
  const endPoint = { lineIndex: (comment.end_line ?? comment.line) - 1, column: comment.end_column };
  ...
  comment.end_line = nextEnd.lineIndex === nextStart.lineIndex ? undefined : Math.max(1, nextEnd.lineIndex + 1);
  comment.end_column = Math.max(0, nextEnd.column);
  return true;
}

export function applyLineShifts(comments: Comment[], changes: readonly EditorContentChange[]): boolean {
  let anyMoved = false;
  ...
  for (const change of sorted) {
    for (const comment of comments) {
      ...
      if (nextStart.lineIndex !== startPoint.lineIndex || nextEnd.lineIndex !== endPoint.lineIndex) {
        comment.line = Math.max(1, nextStart.lineIndex + 1);

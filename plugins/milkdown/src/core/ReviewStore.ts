import { applyReanchorResults, reanchorDocumentLines } from "@mrsf/cli/browser";
...
export class ReviewStore {
  ...
  constructor(
    private readonly host: MilkdownMrsfHostAdapter,
    private readonly options: { showResolved?: boolean } = {},
  ) {}

  getHostAdapter(): MilkdownMrsfHostAdapter {
    return this.host;
  }
  ...
  getThreadsAtLine(resourceId: string, line: number): ReviewThread[] {
    ...
    const commentsById = new Map(state.projectedDocument.comments.map((comment) => [comment.id, comment]));
    ...
    return lineSnapshot.threads
      .map((thread) => {
        ...
        return { line, rootComment, replies } satisfies ReviewThread;
      })
  ...
  async load(resourceId: string, options: ReviewLoadOptions = {}): Promise<ReviewState> {
    ...
    const documentPath = this.host.getDocumentPath ? await this.host.getDocumentPath(resourceId) : null;

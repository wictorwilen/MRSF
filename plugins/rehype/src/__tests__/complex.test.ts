/**
 * Advanced tests for @mrsf/rehype-mrsf plugin using a complex markdown fixture.
 *
...
// Dynamically import parseSidecarContent from @mrsf/cli
...
/** Render arbitrary markdown with a subset of the sidecar comments. */
async function renderWith(
  ...
  const result = await unified()
    ...
    .use(remarkGfm)
    .use(remarkRehype)
    .use(rehypeMrsf, { comments: doc, ...opts } as MrsfPluginOptions)
    .use(rehypeStringify, { allowDangerousHtml: true })
    .process(markdown);
  return String(result);
}
...
/** Count elements with mrsf-line-highlight class. */
...
describe("complex document — headings", () => {
  ...
  it("should annotate h3 heading (line 12)", async () => {
    const html = await renderComplex();
    expect(html).toMatch(/<h3[^>]*data-mrsf-line="12"[^>]*>/);
  });

  it("should not wrap heading content in extra elements", async () => {
    const html = await renderComplex();
    // Heading should be a clean <h1>...</h1> with no injected children
...
describe("complex document — paragraphs", () => {
  ...
  it("should annotate paragraph with inline code (line 7)", async () => {
    const html = await renderComplex();
    expect(html).toMatch(/<p[^>]*data-mrsf-line="7"[^>]*>/);
  });

  it("should preserve inline formatting inside paragraphs", async () => {
    const html = await renderComplex();
  ...
  it("should not inject <mark> for selected_text comments", async () => {
    const html = await renderComplex();
...
describe("complex document — blockquotes", () => {
  it("should annotate blockquote or its children (lines 9-10)", async () => {
    const html = await renderComplex();
    // The blockquote on lines 9-10 should have data-mrsf attributes
    const hasBlockquoteLine = html.match(
      /<blockquote[^>]*data-mrsf-line="9"[^>]*>/,
    );
    const hasParagraphInBq =
      html.includes('data-mrsf-line="9"') ||
      html.includes('data-mrsf-line="10"');
    expect(hasBlockquoteLine || hasParagraphInBq).toBeTruthy();
  });
  ...
  it("should preserve blockquote structure", async () => {
    const html = await renderComplex();
...
describe("complex document — lists", () => {
  it("should annotate unordered list items (line 16)", async () => {
    const html = await renderComplex();
    expect(html).toMatch(/<li[^>]*data-mrsf-line="16"[^>]*>/);
  });

  it("should annotate nested list items (line 19)", async () => {
    const html = await renderComplex();
    expect(html).toMatch(/<li[^>]*data-mrsf-line="19"[^>]*>/);
  });

  it("should annotate ordered list items (line 53)", async () => {
    const html = await renderComplex();
    expect(html).toMatch(/<li[^>]*data-mrsf-line="53"[^>]*>/);
  });

  it("should annotate nested ordered list items (line 56)", async () => {
    const html = await renderComplex();
    expect(html).toMatch(/<li[^>]*data-mrsf-line="56"[^>]*>/);
  });

  it("should annotate deeply nested list item (line 110)", async () => {
    const html = await renderComplex();
    expect(html).toMatch(/<li[^>]*data-mrsf-line="110"[^>]*>/);
  });

  it("should not wrap <li> in extra divs", async () => {
    const html = await renderComplex();
...
describe("complex document — tables", () => {
  it("should produce clean <table> elements", async () => {
    const html = await renderComplex();
  ...
  it("should not inject spans inside table structure", async () => {
    const html = await renderComplex();
    // No mrsf spans directly inside <table>, <thead>, <tbody>
    expect(html).not.toMatch(/<table[^>]*>[\s]*<span/);
    expect(html).not.toMatch(/<thead[^>]*>[\s]*<span/);
    expect(html).not.toMatch(/<tbody[^>]*>[\s]*<span/);
  });

  it("should not inject badges or tooltips inside tables", async () => {
    const html = await renderComplex();
    expect(html).not.toContain("mrsf-badge");
    expect(html).not.toContain("mrsf-tooltip");
  });

  it("should have thread data for table-row comments", async () => {
    const html = await renderComplex();
...
describe("complex document — code fences", () => {
  it("should annotate <pre> for JSON code fence (line 40)", async () => {
    const html = await renderComplex();
    expect(html).toMatch(/<pre[^>]*data-mrsf-line="40"[^>]*>/);
  });

  it("should annotate <pre> for TypeScript code fence (line 62)", async () => {
    const html = await renderComplex();
    expect(html).toMatch(/<pre[^>]*data-mrsf-line="62"[^>]*>/);
  });

  it("should annotate <pre> for YAML code fence (line 78)", async () => {
    const html = await renderComplex();
    expect(html).toMatch(/<pre[^>]*data-mrsf-line="78"[^>]*>/);
  });

  it("should preserve <pre><code> structure", async () => {
    const html = await renderComplex();
    expect(html).toMatch(/<pre[^>]*><code/);
    // No wrapping around the code block
    expect(html).not.toMatch(/<div[^>]*mrsf[^>]*>[\s]*<pre/);
  });

  it("should have code fence comments in thread data", async () => {
    const html = await renderComplex();
    const data = parseDataScript(html)!;
...
describe("complex document — thread data integrity", () => {
  it("should preserve severity across all comments", async () => {
    const html = await renderComplex();
    const data = parseDataScript(html)!;
  ...
  it("should preserve author information", async () => {
    const html = await renderComplex();
    const data = parseDataScript(html)!;
  ...
  it("should exclude resolved comments when showResolved is false", async () => {
    const html = await renderComplex({ showResolved: false });
    const data = parseDataScript(html)!;
    const resolved = data.threads.filter(
      (t: any) => t.comment.resolved === true,
    );
...
describe("complex document — DOM cleanliness", () => {
  ...
  it("should not contain line-row wrappers", async () => {
    const html = await renderComplex();
  ...
  it("should not contain highlight marks", async () => {
    const html = await renderComplex();
    expect(html).not.toContain("<mark");
...
describe("complex document — specific elements", () => {
  it("should handle paragraph with strikethrough (line 114)", async () => {
    const html = await renderComplex();
    // The paragraph on line 114 has ~~2025-12-31~~ — verify the line is annotated
    expect(html).toContain('data-mrsf-line="114"');
    // Verify the strikethrough renders natively (remark produces <del>)
    // Note: remark-parse doesn't support GFM strikethrough by default
    // but the paragraph should still be annotated
  });

  it("should handle the last paragraph (line 118)", async () => {
    const html = await renderComplex();
    expect(html).toContain('data-mrsf-line="118"');
  });

  it("should handle paragraph after horizontal rule (line 30)", async () => {
    const html = await renderComplex();
    expect(html).toContain('data-mrsf-line="30"');
  });

  it("should produce a horizontal rule element", async () => {
    const html = await renderComplex();
    expect(html).toMatch(/<hr/);
  });

  it("should preserve image elements", async () => {
    const html = await renderComplex();
    // Line 116: ![API Architecture](./diagrams/api-arch.png)
    expect(html).toContain("api-arch.png");
    expect(html).toContain("API Architecture");
  });

  it("should preserve link elements in paragraphs", async () => {
    const html = await renderComplex();
    // Line 14: [JWT standard](https://jwt.io)
...
describe("complex document — multi-line blocks", () => {
  it("should set data-mrsf-start-line and data-mrsf-end-line on code fences", async () => {
    const html = await renderComplex();
    // The JSON code fence spans lines 40-47
    expect(html).toMatch(/<pre[^>]*data-mrsf-start-line="40"[^>]*>/);
    expect(html).toMatch(/<pre[^>]*data-mrsf-end-line="47"[^>]*>/);
  });

  it("should set line ranges on blockquotes", async () => {
    const html = await renderComplex();
    // Blockquote spans lines 9-10
    expect(html).toMatch(
      /<blockquote[^>]*data-mrsf-start-line="9"[^>]*>/,
    );
  });

  it("should set line ranges on list containers", async () => {
    const html = await renderComplex();
    // <ul> is not in BLOCK_TAGS, so it doesn't get annotated directly.
    // Instead, the child <li> elements carry line annotations.
    expect(html).toMatch(/<li[^>]*data-mrsf-start-line="16"[^>]*>/);
  });
});
...
describe("complex document — edge cases", () => {
  it("should handle comments on adjacent lines without conflicts", async () => {
    // Lines 9 and 10 are both in a blockquote, but only line 9 has a comment
    const html = await renderComplex();
    const data = parseDataScript(html)!;
    const bqThread = data.threads.find(
      (t: any) => t.comment.id === "bq-note",
  ...
  it("should handle multiple comments from different authors on overlapping lines", async () => {
    // Use fixture subset: two comments on same line from different authors
  ...
  it("should handle deeply nested list comments correctly", async () => {
    const html = await renderComplex();
    const data = parseDataScript(html)!;
  ...
  it("should handle comments targeting both table rows", async () => {
    const html = await renderComplex();
    const data = parseDataScript(html)!;
  ...
  it("should handle all three code fences with different languages", async () => {
    const html = await renderComplex();
    const data = parseDataScript(html)!;
    ...
    // Verify each has the right line
...
describe("complex document — XSS safety", () => {
  it("should escape < in embedded script data", async () => {
    const html = await renderWith("# Title\n", [
    ...
    // Raw < must be escaped as \u003c
    expect(scriptMatch![1]).not.toContain("<");
    // But the data should parse back correctly
  ...
  it("should handle script injection in selected_text", async () => {
    const html = await renderWith("Hello world\n", [
      {
        id: "xss2",
        text: "Note",
        line: 1,

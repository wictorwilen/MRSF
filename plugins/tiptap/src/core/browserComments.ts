...
export async function addComment(doc: MrsfDocument, opts: BrowserAddCommentOptions): Promise<Comment> {
...
export async function populateSelectedText(comment: Comment, documentLines: string[]): Promise<void> {
  ...
  if (startIndex < 0 || endIndex >= documentLines.length) {
    return;
  }

  if (startIndex === endIndex) {
    let line = documentLines[startIndex];
